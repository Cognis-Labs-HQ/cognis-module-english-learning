...
test('registers English surfaces through ctx', () => {
  ...
  const ctx = {
    ...
    router: { get(path) { registrations.routes.push(path); } },
    getCapability(name) { assert.equal(name, 'auth:requireAuth'); return () => ({ role: 'user' }); },
    registerStaticDir() {}, registerSpaRoute(route) { registrations.routes.push(route.base); }, registerNavbarPlugin() {},
    contributePublicCapability(name, value) { registrations.capabilities.push([name, value]); },
    flow: { extend(...args) { registrations.extensions.push(args); } },
  };
  bootstrapModule(ctx);
