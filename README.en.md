...
- English alphabet data and an authenticated alphabet page at `/study/alphabet?language=en`.
- The shared Study library at `/study/library?language=en`.
- A classroom entry point at `/study/en-classroom?language=en`.
...
`bootstrap.js` is the only host integration point. It registers module-owned UI surfaces and ingests its declarative content pack through `ctx`, contributes the English language descriptor as a public capability, and extends the platform bootstrap flow. Runtime code uses repository-relative imports and does not import Cognis internals.

The manifest publishes `ui.stringsBaseUrl` so Cognis can load module-owned translations before the browser UI starts. UI registrations remain scoped so disabling or uninstalling the module leaves no module-owned behavior behind.
