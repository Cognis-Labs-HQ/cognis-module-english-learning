...
- Pronunciation lists and HTTPS audio references without packaged binary media.
- Compound English digraphs with composition links kept separate from localized definitions.
- Capitalized alternate letters declared as variants and consistently positioned to the right of lowercase base cards.
- Numeric alphabet display positions and localized definition-first labels for words, particles, and sentences.
...
`bootstrap.js` is the only host integration point. It ingests its declarative content pack through `ctx`, contributes the English language descriptor as a public capability, and exposes only its locale directory under the module static namespace. It does not extend host UI or bootstrap flows. Runtime code uses repository-relative imports and does not import Cognis internals.
