...
宣言型の英語コンテンツパックを、ホスト提供の `study:library:provider` capability 経由でアトミックに導入します。重複していたライブラリ API、ストア、ページ、ナビゲーション項目は削除し、スキーマ駆動のホスト表示に統一しました。
