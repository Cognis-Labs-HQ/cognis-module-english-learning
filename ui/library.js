...
const root = document.querySelector('#study-language-en-library-root') ?? document.querySelector('main');

async function mount(target) {
  try {
    const response = await fetch('/api/v1/modules/study-language-en/library/snapshot', { credentials: 'same-origin' });
