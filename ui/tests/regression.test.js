...
test('keeps locale keys synchronized', async () => {
  const locales = await Promise.all(['en', 'de', 'id', 'ja'].map((locale) => readFile(`ui/languages/${locale}/strings.xml`, 'utf8')));
  const keys = (xml) => [...xml.matchAll(/name="([^"]+)"/g)].map((match) => match[1]).sort();
