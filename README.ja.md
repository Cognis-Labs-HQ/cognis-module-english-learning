...
- 概念が異なる場合、文脈固有の多言語語彙定義を参照元カードの定義より優先。
- プロバイダー中立のコンテンツ分類、明示的な構成ロール、多言語編集コントロールで最新のライブラリ作成契約に準拠。
- 文ラベル全体を、位置が連続する単語・助詞・句読点の順序付き参照で表現。
- 文字名と一般的な音素の発音一覧、HTTPS 音声参照（バイナリメディアは非同梱）。
- 構成リンクと多言語定義を分離した、英語でよく使われる 7 種類の二重字。
- 大文字を代替形かつ小文字基準カードの空間的な子として、それぞれ明示的に指定。
- 7 列 4 行で読みやすく整理したアルファベット表、数値表示位置、多言語定義優先ラベル。
- アルファベット文字レイヤーだけに限定した簡潔な最小カード。
- ホスト提供の `study:library:provider` capability を通じて取り込まれる宣言型英語コンテンツパック。
...
`bootstrap.js` はホストとの唯一の統合ポイントです。`ctx` を通じて宣言型コンテンツパックを取り込み、英語の言語記述子を公開ケイパビリティとして提供し、モジュールの静的名前空間ではロケールディレクトリだけを公開します。PR #220 がホスト所有の `study:language:en` 名前空間への提供を保護するため、マニフェストはその目的に限って明示的に特権を要求します。ホストルートの登録やセキュリティ上重要なフローの拡張は行いません。ランタイムコードはリポジトリ相対のインポートを使い、Cognis の内部実装をインポートしません。
